...
이번 프로젝트에서 **새로 만든 도메인 서버는 없습니다.** 접수예약, 병원, 검색, 알림 같은 서버는 저장소가 이미 존재했고 일부 신규 기능이 그쪽에서 돌고 있었습니다. 이 프로젝트는 "새 서버 만들기"가 아니라 **`app-server`에 남아 있던 API/Consumer/Batch를 각 도메인 서버로 본격적으로 이관하는 것**이었습니다.
