...
이번 프로젝트에서 **새로 만든 도메인 서버는 없습니다.** 접수예약, 병원, 검색, 알림 같은 서버는 저장소가 이미 존재했고 일부 신규 기능이 그쪽에서 돌고 있었습니다. 이 프로젝트는 "새 서버 만들기"가 아니라 `app-server`에 남아 있던 API/Consumer/Batch를 각 도메인 서버로 본격적으로 이관하는 것이었습니다.
...
`libs/domain`은 도메인 로직을 모아 둔 레이어입니다. DDD의 전술적 패턴이 여기에 그대로 녹아 있습니다.
...
- **Repository** - 영속성 접근을 담당하는 계층. MikroORM 기반 구현이 도메인 레이어 안에 함께 있습니다.
...
export class ReceiptCreateService {
  async createReceipt(body: BulkRequestReceiptBody): Promise<HospitalReservation> {
    // 도메인 로직 한 벌 - API 컨트롤러, Kafka 컨슈머, 배치 어디서든 동일하게 호출
  }
}
```
...
작업을 한 번에 밀지 않고 3단계로 끊었습니다. 한 번에 밀면 되돌릴 여지가 좁고, 운영 부담도 같이 커지기 때문입니다.
...
- **분리를 한 번에 밀지 않고 단계로 끊은 것** - API → 컬렉션 오너십 → DB 순서로 나눈 덕에 매 단계 운영을 안정화한 뒤 다음으로 넘어갈 수 있었습니다. 한 번에 밀었다면 중간에 손을 못 댔을 것 같습니다.
- **배포 후 24~72시간 모니터링을 관례로 만든 것** - "에러가 없다"와 "정상이다"를 구분해서 보게 됐습니다.

아쉬운 쪽.

- **자동화 테스트** - 전환 초기에 더 깔았어야 했습니다. QA 단계에서 뒤늦게 발견된 엣지 케이스 중 상당수는 E2E 테스트로 미리 잡을 수 있었습니다.
- **문서** - 이관 속도를 못 따라갔습니다. 다른 사람이 옮긴 코드를 6개월 뒤 유지보수할 때 왜 이런 선택을 했는지 단서가 남아 있지 않아서, 처음부터 다시 읽어야 하는 경우가 많았습니다.

제일 오래 걸린 논의는 패턴이나 도구 선택이 아니라 어느 팀이 어떤 도메인을 가져갈지였습니다. 경계를 코드로 긋는 것보다 조직으로 긋는 쪽이 훨씬 더뎠습니다.

## 7. 다음 스텝

- **Phase 3 (DB 논리 분리)** - 완료했습니다. 상세는 [MSA - 원칙, 아키텍처, 그리고 DB 분리](/blog/2026/07/09/msa-principles-architecture-db-split/)에 정리했습니다.
