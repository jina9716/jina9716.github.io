...
서비스 데이터를 유관부서에 안정적으로 제공하기 위해 데이터 파이프라인을 구축했고, 이후 Redshift 기반 레거시 구조를 Snowflake로 갈아끼웠습니다. 그 과정과 결과를 기록으로 남깁니다.

## 왜 파이프라인이 필요했는가

유관부서가 서비스 데이터에 접근하는 방식 자체가 문제였습니다.

- 운영 DB 직접 조회 - MongoDB에 그대로 쿼리를 날려 운영 DB에 부하가 갔습니다
- 수동 추출 - 시간이 많이 들고 휴먼 에러가 잦았습니다
- 분석 환경 부재 - 무거운 분석 쿼리를 돌릴 전용 환경이 없었습니다

운영에 영향을 주지 않으면서 자유롭게 데이터를 다룰 수 있는 별도의 분석 환경이 필요했습니다.

## 레거시 아키텍처와 한계

처음에는 Redshift 기반으로 구성했습니다. 흐름은 다음과 같습니다.
...
NestJS Consumer 운영 부담이 컸습니다. 새 컬렉션 하나 붙일 때마다 Entity 정의, Controller 작성, Knex 마이그레이션, 서버 배포까지 코드 레벨 작업이 줄줄이 따라왔습니다. MongoDB의 유연한 스키마를 PostgreSQL의 정적 스키마로 매핑하다 보니 타입 변환 이슈도 잦았고, 배열 필드는 Redshift에서 JSON 문자열로 저장되는데 대소문자 구분이 안 돼서 키를 snake_case로 다 바꿔야 했습니다.

AWS Glue ETL도 만만치 않았습니다. timestamp가 강제로 UTC로 변환되는 이슈, MERGE 모드에서의 정합성 문제 등 Job마다 개별 핸들링이 필요했습니다. Glue Worker 수보다 동시 실행 Job이 많으면 `Max concurrent runs exceeded` 에러가 났습니다. 운영은 Worker 5로 두고 동시 실행 Job 5개 미만으로 시간대를 쪼개서 회피하는 방식이었습니다.

직접적인 트리거는 Redshift 인스턴스 타입 지원 종료였습니다. `dc2.large`가 단종 예정이라 인스턴스 타입을 교체해야 했고, 그러면 비용이 올랐습니다. 한정된 리소스로 인한 성능 저하도 같이 누적되고 있었습니다. 어차피 갈아엎어야 한다면 구조까지 정리하자고 판단했습니다.

## Snowflake 전환 결정

Redshift와 Snowflake를 비교 검토하고 약 3주(2024년 9월 말~10월 중순) PoC를 돌렸습니다.
...
PoC 결과 월 운영 비용은 약 $1,238 → $1,033 으로 17% 정도 줄어드는 것으로 추산됐습니다. 절대 비용도 비용이지만, 제일 크게 끌렸던 건 NestJS Consumer와 AWS Glue를 걷어내고 Snowflake 내장 기능만으로 CDC ETL을 돌릴 수 있다는 점이었습니다.

## 신규 아키텍처

<img src="/assets/images/data-pipeline/architecture-comparison.svg" alt="레거시(Redshift 기반) vs 신규(Snowflake 기반) 데이터 파이프라인 아키텍처 비교" />

NestJS Consumer를 S3 Sink Connector로 대체하면서 파이프라인이 짧아졌습니다. NestJS Consumer + AuroraDB + AWS Glue ETL 세 단계를 걷어내고, Redshift는 Snowflake로 교체했습니다. 코드 배포 없이 커넥터 설정만으로 새 컬렉션을 붙일 수 있게 됐습니다.

### MongoDB Source Connector

MongoDB Change Stream으로 변경 이벤트를 Kafka 토픽에 발행합니다.
...
토픽 컨벤션은 `{prefix}.{database}.{collection}`, Replication Factor 3, 7일 보관으로 잡았습니다.

### S3 Sink Connector - 레거시 Consumer 대체

이전에는 NestJS Consumer가 Kafka 메시지를 받아 AuroraDB에 적재했지만, 이제 S3 Sink Connector가 곧장 S3에 떨굽니다.
...
JSON + gzip 포맷은 Snowflake External Table에서 직접 읽기에 적합하고, `flush.size: 100` + `rotate.schedule.interval.ms: 5000` 조합으로 5초마다 100개 단위로 파일이 떨어집니다. 이걸로 NestJS Consumer 서버 관리, Entity 정의, 마이그레이션, 배포 프로세스를 같이 걷어낼 수 있었습니다.
...
Snowflake에서는 SQL만으로 CDC ETL을 구성할 수 있습니다. 4단계로 풀어 보면 이렇습니다.
...
S3의 JSON 파일을 데이터 이동 없이 Snowflake에서 테이블처럼 조회할 수 있습니다. `cluster_time`을 파싱해두면 이후 MERGE 단계에서 중복 제거 키로 씁니다.
...
Stream은 External Table에 새로 추가된 행을 자동 추적합니다. External Table은 append-only이기 때문에 `INSERT_ONLY = true`를 씁니다.
...
모든 컬럼에 COMMENT를 다는 걸 컨벤션으로 잡았습니다. 유관부서가 별도 문서 없이도 테이블 구조를 이해할 수 있게 하기 위해서입니다. PII(이름, 생년월일, 전화번호, 이메일)에는 마스킹 정책을 걸었습니다.
...
3. `AFTER external_table_refresh_task`로 External Table Refresh 완료 후 실행되도록 의존성을 겁니다

별도 스케줄이 필요한 Task는 CRON으로 독립 실행하면서 내부에서 External Table Refresh를 직접 부릅니다.
...
파이프라인을 깔고 나서는 Snowflake 기능을 같이 붙여가며 분석 환경을 채웠습니다.

### Dynamic Table - 자동 갱신되는 파생 테이블

기본 테이블에 의존하는 파생 테이블을 Dynamic Table로 잡으면 원본이 변할 때 자동으로 증분 처리됩니다. `TARGET_LAG = '5 minutes'` 같은 식으로 갱신 주기를 선언만 하면 됩니다. 별도 통계 배치를 따로 짜고 스케줄을 관리할 필요가 없어진 게 컸습니다.

### Flatten - 중첩 JSON 처리

MongoDB에서 넘어온 도큐먼트는 중첩 JSON이 많습니다. `LATERAL FLATTEN`으로 배열을 정규화된 행으로 풀어낼 수 있었습니다.
...
서비스 특성상 시계열 분석이 잦았는데, 전용 함수가 도움이 됐습니다.
...
Redshift에는 `ASOF JOIN`, `TIME_SLICE` 같은 함수가 없어서 일/월간 집계 쿼리를 직접 풀어 써야 했는데, 이쪽이 훨씬 단순해졌습니다.
...
X-Small Warehouse만으로 대부분의 쿼리가 무리 없이 돌아갔습니다. 동일 배치(4개 테이블 업데이트 SP)가 Redshift에서 40초였는데 Snowflake에서는 5초였습니다. Glue는 Worker 수만큼만 동시 실행이 가능해서 시간대를 쪼개야 했지만, Snowflake Task는 Warehouse queue로 알아서 순차 실행됐습니다.

전환 후 며칠간 Task 실행 이력을 보면 9분 43초, 7분 1초, 7분 30초 정도로 안정적으로 떨어졌습니다.

### 운영 개선

- Snowsight 한 화면에서 쿼리, Task 그래프, 비용 추적까지 봤습니다
- Task History/Task Graph로 의존성과 상태를 시각적으로 추적
- 모니터링 구성을 별도로 짜지 않아도 됐습니다

## 회고

NestJS Consumer와 AWS Glue를 걷어내고 커넥터 + Snowflake 내장 기능으로 대체하면서 운영 부담이 많이 줄었습니다. 새 컬렉션을 붙일 때 코드 배포가 필요 없어진 게 체감상 가장 컸고, Stream + Task + MERGE 조합만으로 별도 ETL 도구 없이 CDC가 돌아가는 것도 덤이었습니다.

MongoDB 쪽 스키마 변경은 여전히 파이프라인에서 제일 까다로운 부분입니다. 어느 필드가 언제 추가/변경되는지 따라가기 어렵고, 운영에서는 이 부분을 관성적으로 체크하는 프로세스가 필요하다고 느꼈습니다.

Kafka lag, S3 적재 상태, Task 실행 이력을 같이 보지 않으면 어디서 막혔는지 잡기 어려웠습니다. 모니터링은 초기부터 같이 깔아두는 편이 낫겠다고 체감했습니다.

이 사례는 Snowflake 데이터 혁신 세미나에서 발표했습니다. 발표 후 질의에서 다른 회사들도 비슷한 지점에서 고민하고 있다는 걸 확인할 수 있었습니다.
